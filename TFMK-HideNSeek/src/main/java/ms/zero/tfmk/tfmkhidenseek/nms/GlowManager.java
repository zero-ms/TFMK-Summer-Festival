...
import static ms.zero.tfmk.tfmkhidenseek.global.GlobalVariable.plugin;
import static ms.zero.tfmk.tfmkhidenseek.global.GlobalVariable.protocolManager;
...
public class GlowManager {
                ...
                if (GameManager.isTagger(packetEvent.getPlayer())) {
                    if (glowedEntityIDs.contains(packetEvent.getPacket().getIntegers().read(0))) {
                        if (packetEvent.getPacketType() == PacketType.Play.Server.ENTITY_METADATA) {
                            ...
                            dataWatcher.setObject(new WrappedDataWatcher.WrappedDataWatcherObject(0, serializer),
                                    (byte) (0x40));
                            packetEvent.getPacket().getWatchableCollectionModifier().write(0,
                                    dataWatcher.getWatchableObjects());
                        }
                    }
                }
            }
    ...
    public static void addToGlowingPool(Player player) {
        glowedEntityIDs.add(player.getEntityId());
        makePlayerGlowing(player);
    }

    public static void removeFromGlowingPool(Player player) {
